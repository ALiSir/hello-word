...
public class MediaProvider {
	...
	public MediaProvider(Context context, int uid) {
	...
	public ArrayList<String> getList(String path, FileFilter type) {
		...
		DeviceManager devMng = new DeviceManager(context, uid);
		int i = 0;
		if (fileList != null) {
			/* 处理多分区的情况 */
//			if (devMng.getLocalDevicesList().contains(path)
//					&& devMng.hasMultiplePartition(path)) {
//				for (i = 0; i < fileList.length; i++) {
//					try {
//						String child = fileList[i].getAbsolutePath();
//						StatFs statFs = new StatFs(child);
//						Log.d("chen",
//								"child:" + child + " block num:"
...
//			}
	...
	public Bitmap getImageThumbFromMK(String origPath, int width, int height) {
		...
		String name = null;
		try {
			name = origPath.substring(origPath.lastIndexOf("/") + 1,
					origPath.lastIndexOf("."));
		} catch (IndexOutOfBoundsException e) {
			e.printStackTrace();
			return null;
		}
