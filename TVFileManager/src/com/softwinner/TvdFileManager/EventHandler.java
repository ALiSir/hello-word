...
public class EventHandler implements 
	OnClickListener,OnItemSelectedListener,OnItemClickListener, OnItemLongClickListener
{
	private static final String TAG = "EventHandler";
	
	public static final String EXTRA_BD_FOLDER_PLAY_MODE = "extra_bd_folder_play_mode";
	
	public static final String SAMBA_AUTH_SHAREDPREFERENCE = "samba_auth_sharedpreference_";
	...
	public String userRootPath = "User://";
	...
	public EventHandler(Context context, final IEventHandlerCallbacks callbacks,
			MediaProvider provider, DeviceManager devices, String username) 
	{
		userName = username;
		mContext   = context;
		...
		if(mDevices.getUserId() == -1) {
			currentDir = userRootPath;
		} else {
			currentDir = rootPath;
		}
		...
		builder.setMessage("此目录为用户私有目录，等待用户授权中...");
		...
		mAuthDialog = builder.create();
	}
	
	private void prepareScanDir() {
		if(currentDir.equals(userRootPath)) {
			getUserList();
	...
	public void onItemClick(final AdapterView<?> parent, final View view, final int position, final long id) {
		...
		final String filePath = mDataSource.get(position);
		...
		if(mDevices.isUser(filePath))
		{
			mDevices.setUserId(mDevices.getUserId(filePath));
			getDeviceList();
			list.setSelection(0);
		...
		else if(mDevices.isSambaPrivateDirPath(filePath))
		{
			final String username = mDevices.getUsername(mDevices.getUserId());
			SharedPreferences sp = mContext.getSharedPreferences(SAMBA_AUTH_SHAREDPREFERENCE + username, Context.MODE_PRIVATE);
			int pos = filePath.lastIndexOf("/");
			String dir = filePath;
			if(pos != -1) {
				dir = filePath.substring(pos+1);
			}
			int auth = -1;
			long auth_time = sp.getLong(filePath + "_auth_time", 0);
			final String clientChannelId = mDevices.getChannelId(username);
			if(TextUtils.isEmpty(clientChannelId)) {
				mAuthDialog.setMessage("这个目录是用户的私有文件夹，需要发送授权请求。但用户未登录过服务器，无法建立通信");
				mAuthDialog.show();
				return;
			}
			Log.e("auth_time", "userName = " + username + ", cur:" + System.currentTimeMillis() + ", auth_time:" + auth_time + ", channelid = " + clientChannelId);
			if(auth_time > 0) {
				Log.e("auth_time", "System.currentTimeMillis() - auth_time = " + (System.currentTimeMillis() - auth_time));
				if(System.currentTimeMillis() - auth_time > mDevices.getAuthExpireTime()) {
					auth = -2;
				} else {
					auth = 1;
				}
			}
			final int _auth = auth;
			final String _dir = dir;
			if(auth < 1) {
				if(auth == -2) {
					mAuthDialog.setMessage("授权已过期，等待用户重新授权中...");
				} else{
					mAuthDialog.setMessage("此目录为用户私有目录，等待用户授权中...");
				}
				mAuthDialog.show();
				new Thread(new Runnable() {
					...
					public void run() {
						//服务端代码调用
						Push.singleAndroidPush(App.getInstance().getChannelId(), clientChannelId,
								"访问授权",
								"有个人想要访问你的" + _dir + "目录，需要你的授权",
								username + ":" + filePath, _auth + "", Push.NOTIFICATION);
						Log.i("Push", "推送服务准备推送内容！");
					}
				}).start();
			} else {
				//FIXME: 每次都重新挂载。原因是系统开机后不会保存已挂载列表。但这样做造成了一个问题，即df查看挂载列表会有重复挂载
				//TODO: 解决方法有两个：一是在APP层手动管理一个挂载列表，当filePath出现在挂载列表，则不挂载。二是在JNI层挂载之前通过df查询filePath是否出现在挂载列表
				mDevices.mountFolder(filePath);
				goIntoDir(filePath);
			}
		}
		else if(mDevices.isSambaPublicDirPath(filePath))
		{
			Log.e(TAG, "go into public dir path:" + filePath);
			goIntoDir(filePath);
		}
		else if(filePath.equals(mContext.getResources().getString(R.string.samba_private_add)))
		{
			if(!mDevices.isSambaOwner()) {
				Toast.makeText(mContext, "您不是这个共享的所有者,无法创建共享!", Toast.LENGTH_SHORT).show();
			} else {
				AddSambaDirDialog2 dialog = new AddSambaDirDialog2(mContext, AddSambaDirDialog2.TYPE_PRIVATE);
				dialog.setCallback(new AddSambaDirDialog2.Callback() {
					@Override
					public void onOk(String dirName) {
						new AsyncTask<String, Void, Integer>() {
							protected Integer doInBackground(String... arg0) {
								int result = mDevices.createSambaPrivateDir(arg0[0]);
								return result;
							}
		...
		else if(filePath.equals(mContext.getResources().getString(R.string.samba_public_add)))
		{
			AddSambaDirDialog2 dialog = new AddSambaDirDialog2(mContext, AddSambaDirDialog2.TYPE_PUBLIC);
			dialog.setCallback(new AddSambaDirDialog2.Callback() {
				@Override
				public void onOk(String dirName) {
					new AsyncTask<String, Void, Integer>() {
						protected Integer doInBackground(String... arg0) {
							int result = mDevices.createSambaPublicDir(arg0[0]);
							return result;
						}
						protected void onPostExecute(Integer result) {
							if(result == 0) {
		...
		else if(filePath.equals(mContext.getResources().getString(R.string.local_home))
			  || filePath.equals(mContext.getResources().getString(R.string.local_user_list))
		      || filePath.equals(mContext.getResources().getString(R.string.samba_public))
		      || filePath.equals(mContext.getResources().getString(R.string.samba_private)))
		{
			return;
		}
		else if(mDevices.isInternalDevicesList(filePath))
		{
			goIntoDir(filePath);
		}
...
//		}
		i = j = 0;
		ArrayList<String> sambaPublicDirList = mDevices.getSambaPublicDirList();
		for(j = 0; j < sambaPublicDirList.size(); j++)
		{
			if(currentDir.equals(sambaPublicDirList.get(j)))
			{
				getDeviceList();
				mTable.notifyDataSetChanged();
				i = mPathStack.pop().intValue();
		...
		i = j = 0;
		ArrayList<String> sambaPrivateDirList = mDevices.getSambaPrivateDirList();
		for(j = 0; j < sambaPrivateDirList.size(); j++)
		{
			if(currentDir.equals(sambaPrivateDirList.get(j)))
			{
				getDeviceList();
				mTable.notifyDataSetChanged();	
				i = mPathStack.pop().intValue();
		...
		if(mDevices.isInternalDevicesList(currentDir)) 
		{
			getUserList();
			mTable.notifyDataSetChanged();
			getDetailForPosition(0);
		...
		/* 返回上一层目录 */
		else if(!currentDir.equals(userRootPath))
		{
			Log.d("","--------back to pre ");
	...
	public boolean goIntoDir(String path)
	{
		...
		if(file.isDirectory())
		{
			...
			{
				Toast.makeText(mContext, mContext.getResources().getString(R.string.can_not_open), Toast.LENGTH_SHORT).show();
				return false;
			}
		}
		else 
	...
	public void getDeviceList() {
		...
		//mDataSource.add(mContext.getResources().getString(R.string.local_home));
		//mDataSource.addAll(mDevices.getInternalDevicesList());
		
		mDataSource.add(mContext.getResources().getString(R.string.samba_public));
		mDataSource.addAll(mDevices.getSambaPublicDirList());
		mDataSource.add(mContext.getResources().getString(R.string.samba_public_add));
		
		mDataSource.add(mContext.getResources().getString(R.string.samba_private));
		mDataSource.addAll(mDevices.getSambaPrivateDirList());
		if(mDevices.isSambaOwner()) {
			mDataSource.add(mContext.getResources().getString(R.string.samba_private_add));
		}
	...
	public void getUserList() {
		...
		currentDir = userRootPath;
		currentPosition = 0;
		if(mDevices.isUserIdEmpty()) {
			mDataSource.add(mContext.getResources().getString(R.string.local_home));
			mDataSource.addAll(mDevices.getInternalDevicesList());
		}
		mDataSource.add(mContext.getResources().getString(R.string.local_user_list));
		mDataSource.addAll(mDevices.getUserList());
		mTable.notifyDataSetChanged();
	}
	...
	public class TableRow extends ArrayAdapter<String>  {
		...
		public final int VIEW_TYPE_BUTTON = 2;
		
    	public TableRow() {
    		super(mContext, R.layout.tablerow_item, mDataSource);
    	}
    	...
    	public int getItemViewType(int position) {
    		...
    		if(path.equals(mContext.getResources().getString(R.string.local_home))) {
    			return VIEW_TYPE_TITLE;
    		}
    		if(path.equals(mContext.getResources().getString(R.string.local_user_list))) {
    			return VIEW_TYPE_TITLE;
    		}
    		if(path.equals(mContext.getResources().getString(R.string.samba_public))) {
    			return VIEW_TYPE_TITLE;
    		}
    		if(path.equals(mContext.getResources().getString(R.string.samba_private))) {
    			return VIEW_TYPE_TITLE;
    		}
    		if(path.equals(mContext.getResources().getString(R.string.samba_public_add))) {
    			return VIEW_TYPE_BUTTON;
    		}
    		if(path.equals(mContext.getResources().getString(R.string.samba_private_add))) {
    			return VIEW_TYPE_BUTTON;
    		}
    	...
    	public View getView(int position, View convertView, ViewGroup parent) {
    		...
    		if(convertView == null) {
    			...
    			if(getItemViewType(position) == VIEW_TYPE_TITLE) {
    				convertView = inflater.inflate(R.layout.tablerow_title, parent, false);
    			} else {
    				convertView = inflater.inflate(R.layout.tablerow_item, parent, false);
    			}
    		...
    		if(mDevices.isInterStoragePath(path)) {
   			...
   			} else if(mDevices.isSambaPrivateDirPath(path) || mDevices.isSambaPublicDirPath(path)) {
    			holder.text.setText(fileName);
    			holder.icon.setImageResource(R.drawable.litter_file);
    		} else if(path.equals(mContext.getResources().getString(R.string.local_home))
    				|| path.equals(mContext.getResources().getString(R.string.local_user_list))
    				|| path.equals(mContext.getResources().getString(R.string.samba_public))
    				|| path.equals(mContext.getResources().getString(R.string.samba_private))
    				) {
    			holder.text.setText(path);
   				holder.icon.setVisibility(View.GONE);
    		} else if(path.equals(mContext.getResources().getString(R.string.samba_public_add))
    				|| path.equals(mContext.getResources().getString(R.string.samba_private_add))) {
    			holder.text.setText(path);
   				holder.icon.setImageResource(R.drawable.litter_samba);
    		} else if(mDevices.isUser(path)) {
    			holder.text.setText(path);
    			holder.icon.setVisibility(View.GONE);
    		}
...
//   				else
	...
	public boolean currentDirIsRootPath() {
		return currentDir.equals(rootPath);
	}
...
//			partition = list[i].substring(list[i].lastIndexOf("/") + 1);
	...
	public void goTheSambaDir(String username, String dir,String content2) {
		if(content2.equals("no")){
			Log.i("mydebug", "拒绝授权！");
			mAuthDialog.setMessage("用户拒绝授权！");
			mAuthDialog.show();
		}else if(content2.equals("yes")){
			mDevices.mountFolder(dir);
			SharedPreferences sp = mContext.getSharedPreferences(SAMBA_AUTH_SHAREDPREFERENCE + username, Context.MODE_PRIVATE);
			SharedPreferences.Editor editor = sp.edit();
			editor.putLong(dir + "_auth_time", System.currentTimeMillis());
			editor.commit();
			mAuthDialog.dismiss();
			Toast.makeText(mContext, "授权成功！", Toast.LENGTH_SHORT).show();
			String path = mDataSource.get(currentPosition);
			Log.e(TAG, "goTheSambaDir = " + path);
			goIntoDir(path);
		}
...
//			else 
	...
	{
		String target = "";
		ArrayList<String> sambaDirList = mDevices.getSambaPrivateDirList();
		for(int i = 0 ; i < sambaDirList.size(); i++)
		{
			String filePath = sambaDirList.get(i);
			if(path.startsWith(filePath)) {
				target = "Samba share";
				if(path.length() >= DeviceManager.SAMBA_PRIVATE_SHARE_DIR_HOME.length()) {
					target = path.substring(DeviceManager.SAMBA_PRIVATE_SHARE_DIR_HOME.length());
...
//			if(path.startsWith(filePath))
//			{
//				if(mDevices.isInterStoragePath(filePath))
//				{
//					target = mContext.getResources().getString(R.string.flash);
//				}
//				else if(mDevices.isSdStoragePath(filePath))
//				{
//					target = mContext.getResources().getString(R.string.sdcard);
//				}
//				else if(mDevices.isUsbStoragePath(filePath))
//				{
//					target = mContext.getResources().getString(R.string.usb);
//				}
//				else if(mDevices.isSataStoragePath(filePath))
//				{
...
//			}
	...
	public boolean isInternalDeviceSelected()
	{
		String name = null;
		if(currentPosition >= 0 && mDataSource.size() > currentPosition)
		{
			name = mDataSource.get(currentPosition);
			if(mDevices.isInternalDevicesList(name))
			{
				return true;
			}
	...
	public void copyFile(String oldLocation, String newLocation)
	{
		 ...
		 */
		if(oldLocation.equals(MediaProvider.RETURN) || newLocation.equals(rootPath) /*
	...
	public class MenuItemListener implements OnClickListener {
		...
		private void saveCopyFilePath(String filePath) {
			...
			try {
				BufferedWriter o_stream = new BufferedWriter(new FileWriter(f,true));
	...
	private class BackgroundWork extends AsyncTask<String, Integer, ArrayList<String>> {
    	...
    	protected void onPreExecute() {
    		...
    		switch(type) {
    			case SEARCH_TYPE:
    				showProgressDialog(R.drawable.icon, "hello", null, ProgressDialog.STYLE_SPINNER, true);
    				break;
    			case COPY_TYPE:
    				String title;
	...
	private boolean playBlurayFolder(String path){
		...
		bdIntent.putExtra(/*MediaStore.EXTRA_BD_FOLDER_PLAY_MODE*/EXTRA_BD_FOLDER_PLAY_MODE , true);
	...
	private void playVideo(String path)
	{
		...
		movieIntent.putExtra(/*MediaStore.EXTRA_BD_FOLDER_PLAY_MODE*/EXTRA_BD_FOLDER_PLAY_MODE , false);
